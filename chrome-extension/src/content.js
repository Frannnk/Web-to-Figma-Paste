(() => {
  ...
  const iconGithub =
    '<svg class="weblink-footer-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 .5C5.649.5.5 5.649.5 12A11.5 11.5 0 0 0 8.36 22.018c.575.106.785-.25.785-.555 0-.274-.01-1-.015-1.962-3.181.691-3.853-1.532-3.853-1.532-.52-1.322-1.27-1.674-1.27-1.674-1.039-.71.078-.696.078-.696 1.148.08 1.752 1.179 1.752 1.179 1.02 1.748 2.675 1.243 3.327.95.104-.739.4-1.244.728-1.53-2.54-.289-5.212-1.27-5.212-5.653 0-1.249.446-2.27 1.178-3.07-.118-.289-.51-1.452.112-3.025 0 0 .96-.307 3.145 1.173A10.95 10.95 0 0 1 12 6.032c.972.005 1.95.131 2.864.384 2.184-1.48 3.143-1.173 3.143-1.173.624 1.573.232 2.736.114 3.025.734.8 1.177 1.821 1.177 3.07 0 4.394-2.677 5.36-5.225 5.644.411.353.778 1.049.778 2.114 0 1.527-.014 2.759-.014 3.133 0 .308.207.667.79.554A11.502 11.502 0 0 0 23.5 12C23.5 5.649 18.351.5 12 .5Z" fill="currentColor"/></svg>';

  let toolbar = null;
  let toolbarHead = null;
  let statusEl = null;
  ...
  function setupToolbar() {
    ...
    toolbar.innerHTML = `
      ...
      <div class="weblink-body">
        ...
        <div class="weblink-footer">
          ...
          <a
            class="weblink-footer-link"
            href="https://github.com/Frannnk/Web-to-Figma-Paste"
            target="_blank"
            rel="noreferrer noopener"
            aria-label="Open GitHub repository and star the project"
          >${iconGithub}<span>Star on GitHub</span></a>
        </div>
      </div>
    `;
    ...
    toolbarHead = toolbar.querySelector('.weblink-head');
    ...
    applyDefaultToolbarPosition();
    toolbarHead?.addEventListener('pointerdown', onToolbarDragStart);
  ...
  function teardownToolbar() {
    ...
    stopToolbarDrag();
    toolbarHead?.removeEventListener('pointerdown', onToolbarDragStart);
  ...
  function onPageClick(event) {
    ...
    exitPickingMode({ keepStatus: true });
  ...
  function onToolbarDragStart(event) {
    if (!toolbar || !toolbarHead) return;
    if (event.target.closest('.weblink-close')) return;
    ...
    document.addEventListener('pointermove', onToolbarDragMove, true);
    document.addEventListener('pointerup', onToolbarDragEnd, true);
    document.addEventListener('pointercancel', onToolbarDragEnd, true);
    event.preventDefault();
  }

  function onToolbarDragMove(event) {
    if (!toolbar || state.dragPointerId !== event.pointerId) return;

    const bounds = getToolbarBounds();
    const nextLeft = clamp(event.clientX - state.dragOffsetX, 8, bounds.maxLeft);
    const nextTop = clamp(event.clientY - state.dragOffsetY, 8, bounds.maxTop);
  ...
  function onToolbarDragEnd(event) {
  ...
  async function copyBySelector(selector, sourceLabel) {
    ...
    const titleLabel = sourceLabel;

    setStatus(`${titleLabel}\n开始采集...`, 'progress');
    await wait(120);
    ...
    try {
      ...
      setStatus(
        `${titleLabel}\n采集完成...\n内容已复制，若图片较多，前往 Figma 粘贴可能需等待几秒。`,
        'success'
      );
  ...
  function toUserFacingError(error) {
    ...
    if (raw === 'EXTENSION_RUNTIME_UNAVAILABLE') {
      return '扩展连接已失效，请刷新当前页面后重试。';
    }
    if (lower.includes('extension context invalidated')) {
      return '扩展已更新或重载，请刷新当前页面后重试。';
    }
    if (lower.includes('cannot read properties of undefined') && lower.includes('sendmessage')) {
      return '扩展已更新或重载，请刷新当前页面后重试。';
    }
    return raw;
  }

  function canSendExtensionMessage() {
  ...
  function enterPickingMode() {
    state.picking = true;
    setStatus('选择模式已开启...\n点击页面任意元素进行采集；再次点击“选择元素”或按 Esc 可取消。', 'progress');
  }

  function exitPickingMode(options = {}) {
  ...
  function stopToolbarDrag() {
    if (toolbarHead && state.dragPointerId !== null) {
      try {
        toolbarHead.releasePointerCapture(state.dragPointerId);
      } catch (error) {
    ...
    document.removeEventListener('pointerup', onToolbarDragEnd, true);
    document.removeEventListener('pointercancel', onToolbarDragEnd, true);
  }

  function applyDefaultToolbarPosition() {
    if (!toolbar) return;

    const bounds = getToolbarBounds();
    toolbar.style.right = 'auto';
    toolbar.style.left = `${bounds.maxLeft}px`;
  ...
  function getToolbarBounds() {
    ...
    return {
      maxLeft: Math.max(8, window.innerWidth - toolbarWidth - 16),
      maxTop: Math.max(8, window.innerHeight - toolbarHeight - 8)
    };
  }
